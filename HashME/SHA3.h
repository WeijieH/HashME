...
#define SHA3_SHFR(x, n) (x >> n)
#define SHA3_ROTR(x, n) ((x >> n) | (x << ((sizeof(x) << 3) - n)))
#define SHA3_ROTL(x, n) ((x << n) | (x >> ((sizeof(x) << 3) - n)))
#define SHA3_CH(x, y, z) ((x & y) ^ (~x & z))
#define SHA3_MAJ(x, y, z) ((x & y) ^ (x & z) ^ (y & z))
#define SHA3_F1(x) (SHA3_ROTR(x, 28) ^ SHA3_ROTR(x, 34) ^ SHA3_ROTR(x, 39))
#define SHA3_F2(x) (SHA3_ROTR(x, 14) ^ SHA3_ROTR(x, 18) ^ SHA3_ROTR(x, 41))
#define SHA3_F3(x) (SHA3_ROTR(x, 1) ^ SHA3_ROTR(x, 8) ^ SHA3_SHFR(x, 7))
#define SHA3_F4(x) (SHA3_ROTR(x, 19) ^ SHA3_ROTR(x, 61) ^ SHA3_SHFR(x, 6))

const unsigned long long SHA3[80] = // ULL = uint64
...
extern unsigned char* sha384_MemBlock(const unsigned char* msg, size_t size, HASH_ctx* ctx);

extern void sha384_final_file(HASH_ctx* ctx, const unsigned char* buf, size_t bufsize);
extern void sha384_update_file(HASH_ctx* ctx, const unsigned char* buf, size_t bufsize);
extern void sha384_init_file(HASH_ctx* ctx, uint64_t filesize);

extern unsigned char* sha512_MemBlock(const unsigned char* msg, size_t size, HASH_ctx* ctx);

extern void sha512_final_file(HASH_ctx* ctx, const unsigned char* buf, size_t bufsize);
extern void sha512_update_file(HASH_ctx* ctx, const unsigned char* buf, size_t bufsize);
extern void sha512_init_file(HASH_ctx* ctx, uint64_t filesize);

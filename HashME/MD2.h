#pragma once
#include "Common.h"
#include "Formats.h"

static const uint8_t MD2[256] = {
    };

extern unsigned char *md2_MemBlock(const unsigned char *buf, size_t size,
                                   HASH_ctx *ctx);

extern void md2_final_file(HASH_ctx *ctx, const unsigned char *buf,
                           size_t bufsize);
extern void md2_update_file(HASH_ctx *ctx, const unsigned char *buf,
                            size_t bufsize);
extern void md2_init_file(HASH_ctx *ctx, uint64_t filesize);
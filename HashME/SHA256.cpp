...
void sha256_process_block(uint32_t hash[8],
                          const uint32_t block[SHA256_BLOCK_SIZE / 4]) {
  uint32_t w[64];
  uint32_t wv[8];
  uint32_t t1, t2;
  int j;
...
void sha256_update(HASH_ctx *ctx, const unsigned char *buf, uint32_t size) {
  ctx->length_ += size;

  while (size >= SHA224_BLOCK_SIZE) {
    sha256_process_block(ctx->SHA256_hash_,
...
void sha256_update_file(HASH_ctx *ctx, const unsigned char *buf,
                        size_t bufsize) {
  size_t i;
...
void sha256_final(HASH_ctx *ctx, const unsigned char *msg, size_t size) {
...
void sha256_final_file(HASH_ctx *ctx, const unsigned char *buf,
                       size_t bufsize) {
...
unsigned char *sha256_MemBlock(const unsigned char *msg, size_t size,
                               HASH_ctx *ctx) {
  sha256_init(ctx);

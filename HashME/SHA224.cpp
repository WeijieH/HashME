...
void sha224_process_block(uint32_t hash[8], const uint32_t block[SHA224_BLOCK_SIZE / 4])
{
	uint32_t w[64];
	uint32_t wv[8];
	uint32_t t1, t2;
...
void sha224_update(HASH_ctx *ctx, 
					...
					uint32_t size)
{
	...
	{
		sha224_process_block(ctx->SHA224_hash_, reinterpret_cast<const uint32_t *>(buf));
...
void sha224_update_file(HASH_ctx *ctx, const unsigned char *buf, size_t bufsize)
{
	size_t i;
...
void sha224_final(HASH_ctx *ctx,
...
void sha224_final_file(HASH_ctx *ctx, const unsigned char *buf, size_t bufsize)
{
...
unsigned char *sha224_MemBlock(const unsigned char *msg,
	size_t size,
	HASH_ctx* ctx)
{
	sha224_init(ctx);

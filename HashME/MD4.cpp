...
void md4_process_block(uint32_t state[4],
                       const uint32_t block[MD4_BLOCK_SIZE / 4]) {
...
void md4_final(HASH_ctx *ctx, const unsigned char *buf, size_t size) {
  ...
  if (ctx->unprocessed_) {
    memcpy(message, buf + size - ctx->unprocessed_,
...
unsigned char *md4_MemBlock(const unsigned char *buf, size_t size,
                            HASH_ctx *ctx) {
  md4_init(ctx);

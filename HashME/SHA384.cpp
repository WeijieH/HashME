...
// size uint64 = 8
void sha384_process_block(uint64_t hash[8], const uint64_t block[SHA384_BLOCK_SIZE / 8])
{
	uint64_t w[80];
	uint64_t wv[8];
	uint64_t t1, t2;	
...
void sha384_update(HASH_ctx *ctx,
	...
	uint32_t size)
{
	...
	{
		sha384_process_block(ctx->SHA384_hash_, reinterpret_cast<const uint64_t *>(buf));
...
void sha384_update_file(HASH_ctx *ctx, const unsigned char *buf, size_t bufsize)
{
	size_t i;
...
void sha384_final(HASH_ctx *ctx,
...
void sha384_final_file(HASH_ctx *ctx, const unsigned char *buf, size_t bufsize)
{
...
unsigned char *sha384_MemBlock(const unsigned char *msg,
	size_t size,
	HASH_ctx* ctx)
{
	sha384_init(ctx);

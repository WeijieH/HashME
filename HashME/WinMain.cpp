...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    wcex.hIconSm = (HICON)LoadImage(hInstance, (LPCTSTR)IDI_ICON1, IMAGE_ICON, 16, 16,
    ...
    HWND hWnd = CreateWindowEx(WS_EX_ACCEPTFILES, szWindowClass, szTitle, WS_POPUPWINDOW | WS_CAPTION | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT,
                               597, 372, NULL, NULL, hInstance, NULL);
...
INT_PTR CALLBACK DlgAboutProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        hWnddlgLOGO = CreateWindow(L"Static", L"", SS_ICON | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE, 270, 15, 128, 128, hdlg, (HMENU)LOGO_dlgwin, hInst, NULL);
        ...
        // STM_SETIMAGE
        //				message was the same bitmap returned by a
        // subsequent STM_SETIMAGE  message. 				The client
        // is responsible to delete any bitmap sent to a static  control.
...
DWORD WINAPI DropHandelThread(LPVOID wParam)
{
    int TotalNumberOfFiles = DragQueryFile((HDROP)wParam, 0xFFFFFFFF, NULL,
...
void HashME_Mem()
{
    ...
    SendMessage(hWndPB, PBM_SETPOS, 0, 0);
    InputTextLen = SendMessage((HWND)hWndInText, WM_GETTEXTLENGTH, (WPARAM)NULL,
    ...
    FormatedTextMemBlock = new unsigned char[4 * InputTextLen + 1]; // Buffer for decode text, *4 for
                                                                    // worst situation, +1 for ending
                                                                    // NULL
    MemCubeLen = FormatTextToMemCube(szInputText, FormatedTextMemBlock, 4 * InputTextLen + 1,
    ...
    if (ST.MD5)
    {
        ...
        uCharToHexStringFormat(ctx_mem.MD5_result, ctx_mem.MD5_HexResult_Output, MD5_HASH_SIZE, MD5_HASH_RESULT_TEXT_SIZE,
...
void InitHashME(HWND hWnd, Settings* ST)
{
    ...
    hWndExcButton = CreateWindow(L"Button", L"HashME", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 520, 10, 50, 20, hWnd, (HMENU)Execute_Button, hInst, NULL);
    SendMessage(hWndExcButton, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndClearButton = CreateWindow(L"Button", Clear_botton_caption, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 10, FunctionButton_Y, 80, 25, hWnd,
                                   (HMENU)Clear_Button, hInst, NULL);
    SendMessage(hWndClearButton, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndStopButton = CreateWindow(L"Button", Stop_botton_caption, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 110, FunctionButton_Y, 80, 25, hWnd,
                                  (HMENU)Stop_Button, hInst, NULL);
    SendMessage(hWndStopButton, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndSettingButton = CreateWindow(L"Button", Setting_botton_caption, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 210, FunctionButton_Y, 80, 25, hWnd,
                                     (HMENU)Setting_Button, hInst, NULL);
    SendMessage(hWndSettingButton, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndAboutButton = CreateWindow(L"Button", About_botton_caption, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 310, FunctionButton_Y, 80, 25, hWnd,
                                   (HMENU)About_Button, hInst, NULL);
    SendMessage(hWndAboutButton, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndInText = CreateWindowEx(WS_EX_CLIENTEDGE | WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR, L"EDIT", L"",
                                ES_LEFT | ES_AUTOHSCROLL | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE, 10, 10, 500, 20, hWnd, (HMENU)Input_Editbox, hInst, NULL);
    ...
    hWndOutText = CreateWindowEx(WS_EX_CLIENTEDGE | WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR, L"EDIT", L"",
                                 ES_LEFT | ES_READONLY | ES_MULTILINE | WS_VSCROLL | WS_HSCROLL | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE, 10, 40, 560,
                                 210, hWnd, (HMENU)Output_Editbox, hInst, NULL);
    SendMessage(hWndOutText, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndPB = CreateWindowEx(WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR, PROGRESS_CLASS, NULL, WS_CHILD | WS_VISIBLE, 10, 295, 560, 15, hWnd,
                            (HMENU)Progress_Bar, hInst, NULL);

    hWndLable =
        CreateWindow(L"Static", Info_lable_caption, SS_LEFT | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE, 10, 315, 560, 20, hWnd, (HMENU)Info_Lable, hInst, NULL);
    SendMessage(hWndLable, WM_SETFONT, (WPARAM)hFont9, TRUE);

    hWndUCButton = CreateWindow(L"Button", Ucase_radbotton_caption, WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 410, FunctionButton_Y + 5, 80, 15,
                                hWnd, (HMENU)UCase_RadButton, hInst, NULL);
    SendMessage(hWndUCButton, WM_SETFONT, (WPARAM)hFont8, TRUE);

    hWndLCButton = CreateWindow(L"Button", Lcase_radbotton_caption, WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON, 490, FunctionButton_Y + 5, 80, 15,
                                hWnd, (HMENU)LCase_RadButton, hInst, NULL);

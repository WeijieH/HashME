...
void CRC_Process(HASH_ctx *ctx, unsigned char *buffer, uint32_t size,
                 Settings *s) {
  if (s->CRC16) {
  ...
  if (s->CRC32) {
    ...
    while (length >= 8) {
      ...
      b = CRC32_Table[7][one & 0xFF] ^ CRC32_Table[6][(one >> 8) & 0xFF] ^
          CRC32_Table[5][(one >> 16) & 0xFF] ^ CRC32_Table[4][one >> 24] ^
          CRC32_Table[3][two & 0xFF] ^ CRC32_Table[2][(two >> 8) & 0xFF] ^
          CRC32_Table[1][(two >> 16) & 0xFF] ^ CRC32_Table[0][two >> 24];
      length -= 8;
    }
